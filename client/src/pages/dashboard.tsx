...
import { Users, MessageSquare, Loader2, TrendingUp, ChevronRight, Sparkles } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  const [showProfessionalDisclaimer, setShowProfessionalDisclaimer] = useState(false);

  // Exibir modal de disclaimer a cada login (removido controle de localStorage)
  useEffect(() => {
    if (currentUser) {
      setShowProfessionalDisclaimer(true);
    }
  }, [currentUser]);

  // Função para fechar o modal (removido localStorage)
  const handleDisclaimerClose = () => {
    setShowProfessionalDisclaimer(false);
  };
  ...
  return (
    <SidebarLayout>
      <div className="p-6 space-y-6">
        ...
        {/* Novo Card de destaque para Chat */}
            ...
            <Link href="/chat">
              <div className="flex items-center justify-between hover:scale-[1.02] transition-transform cursor-pointer">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-lg font-bold text-primary">Nova Interface de Chat</p>
                    <p className="text-sm text-muted-foreground">Acesse todos os módulos em uma conversa unificada</p>
                  </div>
                </div>
            ...
            <CardContent className="space-y-3">
              ...
              <Link href="/dashboard/consulta">
                <div className="flex items-center p-3 bg-green-50 rounded-lg hover:bg-green-100 transition-colors cursor-pointer">
                  ...
                  <div>
                    <p className="font-medium text-green-900">Consulta Clássica</p>
                    <p className="text-sm text-green-700">Interface tradicional com módulos</p>
                  </div>
                </div>
              </Link>
            </CardContent>
      ...
      {/* Modal de disclaimer profissional */}
      <ProfessionalDisclaimerModal
        isOpen={showProfessionalDisclaimer}
